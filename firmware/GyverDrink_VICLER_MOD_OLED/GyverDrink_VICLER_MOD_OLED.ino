/*
  ...
  Исходники на GitHub: https://github.com/VICLER/GyverDrink
   -  OLED дисплей на контроллере SSD1306 (0.96") или SH1106(1.3")(по умолчанию) изменение типа дисплея в файле c_setup в секции // старт дисплея. Дисплей подключается контактами SCK и SDA к пинам А4, А5 Arduino Nano
   ...
   -  возможность изменять настройки прошивки прямо в меню:
      - вход в мену зажатием основной кнопки. Выход либо выбором соответствующего пункта, либо так же зажатием основной кнопки
      - timeout_off: таймаут на выключение дисплея и светодиодов в минутах. Если 0 -> таймаут отключен и светодиоды горят всегда
      - inverse_servo: инвертирсия направление вращения серво. ) -> не инвертированно, 1-> инвертированно
